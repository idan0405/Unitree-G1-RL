...
Hyperparameters tuned to official ManiSkill3 defaults + research best practices.
...
6. Linear LR annealing for stable late-stage convergence
"""
...
# Tuned for high-DOF (25-DOF) dexterous manipulation:
# 1. Massive parallelization (512 envs) to discover rare grasp states.
# 2. Large minibatches (~12.8k transitions) to reduce gradient variance.
# 3. Tighter KL target to prevent destructive policy updates.
# ===========================================================================
ENV_ID = "UnitreeG1PlaceAppleInBowl-v1"
TOTAL_TIMESTEPS = 100_000_000  # 100M: this is a hard multi-phase task
LEARNING_RATE = 3e-4           # standard; linearly annealed to 0
NUM_ENVS = 512                 # parallel envs (reduce if you hit GPU OOM)
NUM_EVAL_ENVS = 10
NUM_STEPS = 100                # rollout horizon per env (matches max_episode_steps)
NUM_EVAL_STEPS = 100
GAMMA = 0.8                    # ManiSkill manipulation default (short fixed-horizon episodes)
GAE_LAMBDA = 0.9               # ManiSkill manipulation default for advantage estimation
NUM_MINIBATCHES = 4            # few, large minibatches (~12.8k transitions each)
UPDATE_EPOCHS = 4              # standard for large minibatches
CLIP_COEF = 0.2
ENT_COEF = 0.01                # needed to escape reaching-only local optimum
VF_COEF = 0.5
MAX_GRAD_NORM = 0.5
TARGET_KL = 0.1                # tighter KL: prevents destructive updates in 25-DOF
REWARD_SCALE = 1.0
SEED = 1
EVAL_FREQ = 25                 # evaluate every 25 iterations
FINITE_HORIZON_GAE = True      # correct for fixed-length episodes (max_episode_steps=100)
ANNEAL_LR = True               # linear decay → stable late-stage convergence
PARTIAL_RESET = True           # auto-reset on termination (not truncation)
SAVE_FREQ = 25                 # save checkpoint every 25 iterations

BATCH_SIZE = NUM_ENVS * NUM_STEPS               # 512 * 100 = 51,200
MINIBATCH_SIZE = BATCH_SIZE // NUM_MINIBATCHES  # 51,200 / 4 = 12,800
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.actor_logstd = nn.Parameter(torch.zeros(1, act_dim))  # std=1.0: WIDE exploration for 25-DOF dexterous hand
...
def main():
    ...
    env_kwargs = dict(
        obs_mode="state",
        control_mode="pd_joint_delta_pos",  # explicit: delta position control (official baseline default)
    ...
    for iteration in range(1, NUM_ITERATIONS + 1):
        ...
        for step in range(NUM_STEPS):
            ...
            action_penalty = torch.sum(action**2, dim=-1) * 0.01
            rewards[step] = (reward.view(-1) - action_penalty) * REWARD_SCALE
            ...
            if "final_info" in infos:
                ...
                # CRITICAL: Bootstrap value from final_observation ONLY for truncated episodes
                with torch.no_grad():
                    valid_trunc = torch.logical_and(done_mask, truncations)
                    if valid_trunc.any():
                        final_values[step, torch.arange(NUM_ENVS, device=device)[valid_trunc]] = \
                            agent.get_value(infos["final_observation"][valid_trunc]).view(-1)
